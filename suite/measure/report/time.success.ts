...
export function reportSuccess(algorithm: Algorithm, iterations: number, timestamp: string, message: ChildMessage, options: RunOptions): void {
  ...
  console.log(`Details:
 • Total Iterations: ${cyan(iterations)}
 ...
 • Average Duration: ${cyan(ms(averageDuration))}`);
  try {
    ...
    appendFileSync(reportFilePath, `${timestamp},${iterations},${totalDuration},${minDuration},${maxDuration},${averageDuration}\n`);
